...
@dbt_assets(manifest=airbnb_project.manifest_path, select="fct_reviews", partitions_def=daily_partitions, dagster_dbt_translator=CustomDagsterDbtTranslator())
def airbnb_dbt_assets_with_custom_translator(context: AssetExecutionContext, dbt: DbtCliResource):
    ...
    dbt_cli_tasks = dbt.cli(dbt_args, context=context, raise_on_error=False)

    yield from dbt_cli_tasks.stream()
